...
default repository `ghcr.io/wso2/api-platform`, tagged with the `gateway.version` that
`build.yaml` declares (this distribution pins **1.2.1**):

```
ghcr.io/wso2/api-platform/<IMAGE_NAME>-gateway-controller:1.2.1
ghcr.io/wso2/api-platform/<IMAGE_NAME>-gateway-runtime:1.2.1
```
...
```yaml
  gateway-controller:
    image: ghcr.io/wso2/api-platform/<IMAGE_NAME>-gateway-controller:1.2.1
  gateway-runtime:
    image: ghcr.io/wso2/api-platform/<IMAGE_NAME>-gateway-runtime:1.2.1
```
...
| **Custom image names in `docker-compose.yaml`** | gateway Compose | The shipped file names stock images; after Step 3 it must name your `<IMAGE_NAME>-gateway-{controller,runtime}:1.2.1` |
